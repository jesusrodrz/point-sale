...
function loadCustomers(page) {
    $.ajax({
        ...
        switch (data.accion) {
            case "success":
                stateCustomers.page = data.recordsTotal % 50 ? (data.recordsTotal / 50).toFixed() + 1 : data.recordsTotal / 50;
...
const clusterizeCustomers = new Clusterize({
    ...
    callbacks: {
        scrollingProgress: (progress) => {
            this.pages = stateCustomers.page;
            if (!this.currentPage) {

                this.currentPage = 1
            }

            console.log('Progress: ' + progress.toFixed(2), 'pages: ' + this.pages, 'page: ' + this.currentPage)

            if (this.currentPage < this.pages && progress.toFixed(2) > 60) {
                loadCustomers(this.currentPage)
                this.currentPage++
            }
        }
    }
...
function printProductgGrid(products) {
    ...
    products.map(product => {
      ...
      <div class="card-price" data-discount="-${
            ...
            }%">S/. <span class="price">${product.PrecioVenta}</span></div>
      <div class="card-discount">
        S/. <span data-discount-unit="${
            product.MontoDescuento
